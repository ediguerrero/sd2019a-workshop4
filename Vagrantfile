...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  config.vm.define :load_balancer do |lb|
    ...
    lb.vm.provider :virtualbox do |vb|
      #vb.customize ["modifyvm", :id, "--memory", "256","--cpus", "1", "--name", "load_balancer" ]
     vb.memory = "256"
     vb.cpus = "1"
 ...
 #lb.vm.provision "shell", inline: <<-SHELL
  ...
  config.vm.define :discovery_service do |ds|
    ...
    ds.vm.provider :virtualbox do |vb|
      #vb.customize ["modifyvm", :id, "--memory", "256","--cpus", "1", "--name", "consul_server" ]
     vb.memory = "256"
     vb.cpus = "1"    
end
  ...
  config.vm.define :microservice_a do |ma|
    ...
    ma.vm.provider :virtualbox do |vb|
     # vb.customize ["modifyvm", :id, "--memory", "256","--cpus", "1", "--name", "microservice_a" ]
     vb.memory = "256"
     vb.cpus = "1"   
 ...
 #     yum install -y wget
  ...
  config.vm.define :microservice_b do |mb|
    ...
    mb.vm.provider :virtualbox do |vb|
     # vb.customize ["modifyvm", :id, "--memory", "256","--cpus", "1", "--name", "microservice_b" ]
     vb.memory = "256"
     vb.cpus = "1"    
end
